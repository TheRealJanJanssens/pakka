(function(global) {
  ...
  var basicCSS = '.editor-icon-picker{transition:all 0.75s cubic-bezier(0.23, 1, 0.32, 1); -webkit-transition:all 0.75s cubic-bezier(0.23, 1, 0.32, 1); -moz-transition:all 0.75s cubic-bezier(0.23, 1, 0.32, 1); -o-transition:all 0.75s cubic-bezier(0.23, 1, 0.32, 1);} .icon-picker { display: inline-block; position: absolute; z-index: 100; padding: 5px; max-height:82px; overflow: scroll; background-color: #000; box-shadow: 0px 32px 32px 0px rgba(20,44,26,0.05), 0px 16px 16px 0px rgba(20,44,26,0.05), 0px 8px 8px 0px rgba(20,44,26,0.05), 0px 4px 4px 0px rgba(20,44,26,0.05), 0px 2px 2px 0px rgba(20,44,26,0.05); border-radius:12px; transition:all 0.75s cubic-bezier(0.23, 1, 0.32, 1); -webkit-transition:all 0.75s cubic-bezier(0.23, 1, 0.32, 1); -moz-transition:all 0.75s cubic-bezier(0.23, 1, 0.32, 1); -o-transition:all 0.75s cubic-bezier(0.23, 1, 0.32, 1);} .icon-picker-single-icon { display: inline-block; width: 20px; height: 20px; margin: 1px; } .icon-picker-single-icon i{pointer-events:none; color:#ccc !important;} .icon-picker-single-icon-picked i{ color: #fff !important;} #icon-picker-search{ position:relative; margin-bottom: 5px;} #icon-picker-search i{font-size: 12px; position:absolute; display: block; top: 50%; -webkit-transform: translateY(-50%); -ms-transform: translateY(-50%); transform: translateY(-50%); left: 5px;} #icon-picker-search input{ font-size:14px; padding:5px; padding-left:20px; height: 25px;background: transparent; color: white; border: 1px solid #ccc; border-radius: 5px;}';
  function singleIconTpl(icon, index, picked) {
    var pickedClass = picked ? "icon-picker-single-icon-picked" : '';
    return '<div class="icon-picker-single-icon ' + pickedClass + '" tabindex="' + index + '" data-icon="' + icon + '"><i class="'+icon+'"></i></div>';
  }
  var DEFAULT_ICONS = [
  ...
  function addBasicStyling() {
    if (document.getElementById('icon-picker-style')) {
      return;
    }
    var style = document.createElement('style');
    style.setAttribute('type', 'text/css');
    style.setAttribute('id', 'icon-picker-style');
  ...
  function SinglePicker(elem, icons, className) {
    ...
    this._positionPickerElement = function() {
      ...
      this.elem.style.left = '0px'; //left + ' px';
      this.elem.style.top = '-90px'; //(top + height) + 'px';
    };
    ...
    this._createPickerElement = function() {
      ...
      this.elem.classList.add('icon-picker');
      if (className) {
        this.elem.classList.add(className);
      }

      var currentlyChosenIconIndex = icons.indexOf(this.targetElem.dataset.customPickerIcon);
	  

	  //adds searchbar
	  //this.elem.innerHTML += "<div id='icon-picker-search' tabindex='"+currentlyChosenIconIndex+"'><i class='fa fa-search'></i><input type='text'></div>";

	  
	  //adds all icons
	  for (var i = 0, len = icons.length; i < len; i++) {
        this.elem.innerHTML += singleIconTpl(icons[i], i + 1, i == currentlyChosenIconIndex);
      }
      
      this.targetElem.parentNode.appendChild(this.elem);
      this.elem.setAttribute('tabindex', 1);

      var toFocus = currentlyChosenIconIndex > -1 ? currentlyChosenIconIndex : 0;
    ...
    this._addEventListeners = function() {
      ...
      this.elem.addEventListener('click', function(e) {
        if (e.target.classList.contains('icon-picker-single-icon')) {
          _this.emit('iconChosen', e.target.dataset.icon);
        }
      });
  ...
  function PickerHolder(elem) {
    ...
    this._addEventListeners = function() {
      ...
      this.on('customIcons', function(icons) {
        if (!(icons instanceof Array)) {
          throw new Error('Icons must be an array');
        }
        this_.icons = icons;
      });
      this.on('defaultIcon', function(icon) {
        if (!this_.elem.dataset.customPickerIcon) {
          this_._updateElemState(icon);
          this_.emit('iconChosen', icon, this_.elem);
        }
      });
    ...
    this._updateElemState = function(icon) {
      this.elem.dataset.customPickerIcon = icon;
      this.elem.value = icon;
    };
    ...
    this.openPicker = function() {
      ...
      this_.currentPicker = new SinglePicker(this_.elem, this_.icons, this_.className);
      this_.currentPicker.on('iconChosen', function(icon) {
        this_._updateElemState(icon);
        this_.destroyPicker();
        this_.emit('iconChosen', icon, this_.elem);
      });
  ...
  function customIconPicker(element, options) {
    // @todo: move from here
    addBasicStyling();
  ...
  if (global.define && global.define.amd) {
    ...
    global.customIconPicker = customIconPicker;
  }
})(this || window);