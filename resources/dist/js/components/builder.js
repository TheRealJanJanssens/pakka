...
// 			$(".se-edit-menu").removeClass("active");
	...
	*/
    ...
    const contentObserver = new MutationObserver(handleMutations);

    // Select all contenteditable elements to observe
    document.querySelectorAll('[contenteditable="true"]').forEach(element => {
        contentObserver.observe(element, observerConfig);
    });

    // Delay function for autosave
    function delay(callback, ms) {
        let timer = 0;
    ...
    // Register a delayed save function on contenteditable elements
    document.querySelectorAll('[contenteditable="true"]').forEach(element => {
        element.addEventListener('input', delay(() => {
            saveContent();
        }, 5000));
    });
