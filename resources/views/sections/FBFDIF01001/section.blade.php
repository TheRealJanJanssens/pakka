...
<!-- 		    	<canvas id="c"></canvas> -->
	    ...
	    @if(checkContent($section['FBFDIF01001_H'], 'title'))
	    	<h4>{{ parseContent($section['FBFDIF01001_H'],'title') }}</h4>
	    @endif
		...
		@if(isset($settings['social_facebook']))
            <a href="{{ $settings['social_facebook'] }}" class="btn btn--primary type--uppercase font-weight-bold e {{ parseSecAttr('.e', $section['classes']) }}">
			    <span class="btn__text"><i class="fa fa-facebook icon icon--xs btn__icon mr-3"></i>{{ parseContent($section['FBFDIF01001_H'],'button') }}</span>
			</a>
        @endif
