...
<head>
  ...
  <body>
				...
				<tr>
					<td valign="top" style="width:50%">
						@if(isset($settings['app_logo']))
			            	<img class="logo" src="{{ base_path() }}{{ config('image.app.public') }}{{ $settings['app_logo'] }}">
			            @else
			            	<img class="logo" src="{{ base_path() }}{{ config('placeholders.logo') }}">
			            @endif
		...
		<table class="f-12" style="margin-top: 60px;">
			<tbody>
				<tr class="p-10">
					<td class="p-10 border" valign="top" style="width:33.33%;">
						<p class="f-12 mb-5 f-light"><b>Bedrijfsinfo</b></p>
					...
					<td class="p-10 border" valign="top" style="width:33.33%;">
						<p class="f-12 mb-5 f-light"><b>Klantgegevens</b></p>
						@if(!empty($invoice['ship_address']))
							<p class="f-12 mb-5 f-light"><b class="d-b f-light">{{ trans('pakka::app.invoiced_to') }}:</b></p>
							...
							<p class="f-12 mb-5 f-light"><b class="d-b f-light" style="margin-top: 20px;">{{ trans('pakka::app.sended_to') }}:</b></p>
					...
					<td class="p-10 border" class="t-r" valign="top" style="width:33.33%;">
						<p class="f-12 mb-5 f-light"><b>{{ $invoice['type'] }} gegevens</b></p>
						<p><b>{{ trans('pakka::app.document_no') }}:</b> {{ $invoice['invoice_no'] }}</p>
						<p><b>{{ trans('pakka::app.date') }}:</b> {{ $invoice['date'] }}</p>
						<p><b>{{ trans('pakka::app.expiration_date') }}:</b> {{ $invoice['due_date'] }}</p>
					</td>
				</tr>
			</tbody>
		</table>
		...
		@if($type == 1)
			<p class="mb-15">Wij danken u voor uw bestelling. Hieronder vind u de omschrijving van uw bestelling.</p>
		@endif
		
		@if($type == 3 || $type == 4)
			<p class="mb-15">Geachte,<br>Met genoegen bieden wij u vrijblijvend onze prijsopgave aan onder onze verkoopsvoorwaarde.</p>
		@endif
		
	   <table class="table invoice-table table-condensed">
			<thead>
                <tr>
	                <td class="text-center" width="50%"><strong>{{ trans('pakka::app.description') }}</strong></td>
					<td><strong>{{ trans('pakka::app.quantity') }}</strong></td>
					<td class="text-center"><strong>{{ trans('pakka::app.unit_price') }}</strong></td>
					<td class="text-right"><strong>{{ trans('pakka::app.vatper') }}</strong></td>
					<td class="text-right" width="15%"><strong>{{ trans('pakka::app.total') }}</strong></td>
                </tr>
			</thead>
			<tbody class="f-12 border">
				...
				<tr>
					<td class="thick-line" rowspan="5" colspan="3"></td>
				...
				<tr>
					<td class="no-line text-center f-14"><strong>{{ trans('pakka::app.total') }}</strong></td>
					<td class="no-line text-right f-14">
						@if($invoice['total'] < 0)
   	  	...
   	  	<div>
	   	  	<h3 style="margin:20px 0px;">Omschrijving:</h3>
	   	  	<p class="f-12">{{ $invoice['description'] }}</p>
	   	  	<p class="f-12">{{ trans('pakka::app.iban') }}: {{ $settings['company_iban'] }}</p>
	   	  	
			@if($type == 3 || $type == 4)
				<p>De bovenvermelde prijzen kunnen ten alletijde aangepast worden en zijn ondervoorbehoud.</p>
			@endif
