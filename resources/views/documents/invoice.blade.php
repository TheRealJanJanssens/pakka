          ...
          <tr>
			...
			<tbody class="f-12">
				...
				<tr>
					...
					<td class="thick-line text-right">
						@if($invoice['subtotal'] < 0)
							{!! str_replace('-','-'.$settings['invoice_valuta'],formatNumber($invoice['subtotal'])) !!}
						@else
							{{$settings['invoice_valuta']}} {{ formatNumber($invoice['subtotal']) }}
						@endif
					</td>
				</tr>
				<tr>
					...
					<td class="no-line text-right">
						@if($invoice['vattotal'] < 0)
							{!! str_replace('-','-'.$settings['invoice_valuta'],formatNumber($invoice['vattotal'])) !!}
						@else
							{{$settings['invoice_valuta']}} {{ formatNumber($invoice['vattotal']) }}
						@endif
					</td>
				</tr>
				<tr>
					...
					<td class="thick-line text-right f-14">
						@if($invoice['total'] < 0)
							{!! str_replace('-','-'.$settings['invoice_valuta'],formatNumber($invoice['total'])) !!}
						@else
							{{$settings['invoice_valuta']}} {{ formatNumber($invoice['total']) }}
						@endif
					</td>
				</tr>
			</tbody>
   	  	...
   	  	<div>
	   	  	...
	   	  	<p class="f-12">{!! nl2br(e($invoice['description'])) !!}</p><br>
	   	  	
	   	  	@if($key !== 4)
<!-- 	   	  		<p class="f-12">{{ trans('pakka::app.make_payment') }} <br>{{ trans('pakka::app.iban') }}: {{ $settings['company_iban'] }}</p> -->
	   	  	@endif

	   	  	<p class="f-12">{!! nl2br(e($settings['invoice_general_description'])) !!}</p>
   	  	</div>
