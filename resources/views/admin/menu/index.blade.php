...
@section('content')
    ...
    <div class="mB-20">
        ...
        <a href="{{ route(config('pakka.prefix.admin').'.menu_items.create') }}" class="btn btn-info">
                ...
                @foreach ($menus as $menu)
                    ...
                    @foreach ($menu['items'] as $item)
	                    	...
	                    	<tr class="item" data-id="{{ $item['id'] }}" data-menu="{{ $menu['id'] }}" data-level="1" data-position="" data-parent="{{ $item['parent'] }}">
		                        <td><i class="handle ti-line-double"></i><a href="{{ route(config('pakka.prefix.admin').'.menu_items.edit', $item['id']) }}">{{ $item['name'] }}</a></td>
		                        <td>
		                            <ul class="list-inline">
		                                <li class="list-inline-item">
		                                    <a href="{{ route(config('pakka.prefix.admin').'.menu_items.edit', $item['id']) }}" title="{{ trans('pakka::app.edit_title') }}" class="btn btn-primary btn-sm"><span class="ti-pencil"></span></a></li>
		                                <li class="list-inline-item">
		                                    {!! Form::open([
		                                        'class'=>'delete',
		                                        'url'  => route(config('pakka.prefix.admin').'.menu_items.destroy', $item['id']),
		                                        'method' => 'DELETE',
		                                        ])
		                                    !!}
		                    ...
		                    @if (isset($item['items']))
				                    ...
				                    @foreach ($item['items'] as $lvlTwoItem)
				                    	...
				                    	<tr class="item" data-id="{{ $lvlTwoItem['id'] }}" data-menu="{{ $menu['id'] }}" data-level="2" data-position="" data-parent="{{ $lvlTwoItem['parent'] }}">
					                        <td><i class="handle ti-line-double"></i><a href="{{ route(config('pakka.prefix.admin').'.menu_items.edit', $lvlTwoItem['id']) }}">{{ $lvlTwoItem['name'] }}</a></td>
					                        <td>
					                            <ul class="list-inline">
					                                <li class="list-inline-item">
					                                    <a href="{{ route(config('pakka.prefix.admin').'.menu_items.edit', $lvlTwoItem['id']) }}" title="{{ trans('pakka::app.edit_title') }}" class="btn btn-primary btn-sm"><span class="ti-pencil"></span></a></li>
					                                <li class="list-inline-item">
					                                    {!! Form::open([
					                                        'class'=>'delete',
					                                        'url'  => route(config('pakka.prefix.admin').'.menu_items.destroy', $lvlTwoItem['id']),
					                                        'method' => 'DELETE',
					                                        ])
					                                    !!}
					                    ...
					                    @if (isset($lvlTwoItem['items']))
							                    ...
							                    @foreach ($lvlTwoItem['items'] as $lvlThreeItem)
							                    	...
							                    	<tr class="item" data-id="{{ $lvlThreeItem['id'] }}" data-menu="{{ $menu['id'] }}" data-level="3" data-position="" data-parent="{{ $lvlThreeItem['parent'] }}">
								                        <td><i class="handle ti-line-double"></i><a href="{{ route(config('pakka.prefix.admin').'.menu_items.edit', $lvlThreeItem['id']) }}">{{ $lvlThreeItem['name'] }}</a></td>
								                        <td>
								                            <ul class="list-inline">
								                                <li class="list-inline-item">
								                                    <a href="{{ route(config('pakka.prefix.admin').'.menu_items.edit', $lvlThreeItem['id']) }}" title="{{ trans('pakka::app.edit_title') }}" class="btn btn-primary btn-sm"><span class="ti-pencil"></span></a></li>
								                                <li class="list-inline-item">
								                                    {!! Form::open([
								                                        'class'=>'delete',
								                                        'url'  => route(config('pakka.prefix.admin').'.menu_items.destroy', $lvlThreeItem['id']),
								                                        'method' => 'DELETE',
								                                        ])
								                                    !!}
